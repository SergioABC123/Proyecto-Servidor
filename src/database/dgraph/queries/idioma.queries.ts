...
export async function buscarOCrearIdioma(nombreIdioma: string): Promise<string> {
  ...
  try {
    ...
    if (data.idioma && data.idioma.length > 0) { // si encontramos un idioma
...
async function buscarUidUsuarioPorMongoId(mongoId: string): Promise<string> { // ahora buscamos al usuario
...
export async function sincronizarIdiomasUsuario(
  mongoId: string,
  idiomas: string[]
): Promise<void> {
  try {
