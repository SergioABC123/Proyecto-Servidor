...
// Cliente gRPC 
const clientStub = new dgraph.DgraphClientStub( // creamos la conexion principal
...
async function setSchema(): Promise<void> { // funcion asincronica para usar el await, Promise<void> es porque la promesa no devuelve valor 
  const schemaPath = path.join(__dirname, "schemas", "schema.graphql"); // le decimos donde esta nuestro esquema
